    ...
    <>
      ...
      <div className="p-4 md:p-8 grid lg:grid-cols-2 gap-6">
        <div className="space-y-4">
              ...
              <div className="border-2 border-dashed border-white/15 rounded-xl p-6 text-center cursor-pointer hover:border-brand/40 transition-colors">
                {preview ? (
                  ...
                  <div className="text-gray-500">
                    ...
                    <div className="text-sm">Click to upload note image</div>
                    <div className="text-xs mt-1">JPG / PNG · capture in good light</div>
                  </div>
                )}
          ...
          <div className="rounded-xl border border-white/8 bg-ink-700 p-5">
            <div className="text-sm font-600 text-gray-300 mb-1">Confirm visible security features</div>
            <p className="text-xs text-gray-500 mb-3">Tick each genuine feature the operator can verify on the note.</p>
            ...
            <button onClick={run} disabled={loading}
              ...
              <ScanLine size={16} /> {loading ? 'Screening…' : 'Screen note'}
            </button>
        ...
        <div ref={resultRef} className="space-y-4 scroll-mt-24">
            ...
            <>
              <div className={`rounded-xl border p-6 ${res.risk_level === 'HIGH' ? 'border-red-500/40 bg-red-500/5' : res.risk_level === 'MEDIUM' ? 'border-yellow-500/40 bg-yellow-500/5' : 'border-emerald-500/40 bg-emerald-500/5'}`}>
                ...
                <div className="mt-2 flex items-end gap-2">
                  ...
                  <span className="text-gray-500 mb-1">/ 100 counterfeit risk</span>
                </div>
              ...
              <div className="rounded-xl border border-white/8 bg-ink-700 p-5">
                <h3 className="font-display font-600 text-white mb-3">Contributing factors</h3>
                <div className="space-y-2">
                  {res.factors.map((f, i) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-emerald-500/20 bg-ink-700 p-4">
                  <div className="text-xs text-emerald-400 font-600 mb-2 flex items-center gap-1"><CheckCircle2 size={14} /> Confirmed</div>
                ...
                <div className="rounded-xl border border-red-500/20 bg-ink-700 p-4">
                  <div className="text-xs text-red-400 font-600 mb-2 flex items-center gap-1"><XCircle size={14} /> Missing / unverified</div>
              ...
              {res.forensics?.available && (
                <div className="rounded-xl border border-white/8 bg-ink-700 p-5 text-xs text-gray-400">
                  <div className="font-600 text-gray-300 mb-2">Image forensics</div>
          ...
          {!res && !loading && (
            <div className="rounded-xl border border-dashed border-white/10 bg-ink-700/50 p-10 text-center text-gray-500">
              ...
              Upload a note, confirm visible features, then <span className="text-brand">Screen note</span>.
            </div>
          )}
        </div>
      </div>
